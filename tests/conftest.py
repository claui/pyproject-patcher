...
@pytest.fixture(name='toml_with_package_data_included')
def fixture_toml_with_package_data_included(tmp_path: Path) -> Path:
    path = tmp_path / 'pyproject_toml'
    with open(path, encoding='utf-8', mode='w') as file:
        file.write(
            dedent("""\
            ...
            [project]
            name = "toml_with_package_data_included"
            version = "0.1.0"
            description = "Test"

            [tool.setuptools]
            include-package-data = true  # this is the default
            """)
        )
    return path
